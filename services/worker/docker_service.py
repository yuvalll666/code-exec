...
async def run_container_job(code: str, req_id: str):
    ...
    filename = f"{req_id}.py"
    worker_file_path = os.path.join("/tmp/shared_jobs", filename)
    ...
    def docker_run():
        ...
        try:
            ...
            volumes = {'sandbox_data': {'bind': '/app', 'mode': 'ro'}}
            ...
            if container:
                try:
                    ...
                    logger.error(f"[{req_id}] Failed to remove container: {e}")
            
            if os.path.exists(worker_file_path):
                    try:
                        os.remove(worker_file_path)
                    except:
                        logger.error(f"[{req_id}] Failed to remove temp file: {e}")
